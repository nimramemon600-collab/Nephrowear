...
def inject_into_html(data):
    ...
    try:
        ...
        utc_time = datetime.strptime(data["last_updated"], "%Y-%m-%d %H:%M UTC")
        edt_time = utc_time.replace(tzinfo=timezone.utc).astimezone(edt)
        updated = edt_time.strftime("%Y-%m-%d %I:%M %p EDT")
